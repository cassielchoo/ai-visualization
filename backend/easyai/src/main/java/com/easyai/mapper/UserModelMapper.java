...
public interface UserModelMapper {
    ...
    @Insert("insert into dbModelInfo(modelId,modelName,userId,dataJson,modelURL,lastEditTime,creatTime,thumbnailUrl,isFavourite,isShare) values (#{modelId},#{modelName},#{userId},#{dataJson},#{modelURL},#{lastEditTime},#{creatTime},#{thumbnailUrl},#{isFavourite},#{isShare});")
    ...
    @Update("update dbModelInfo set modelName=#{modelName},userId=#{userId},dataJson=#{dataJson},modelURL=#{modelURL},lastEditTime=#{lastEditTime},creatTime=#{creatTime},thumbnailUrl=#{thumbnailUrl},isFavourite=#{isFavourite},isShare=#{isShare} where modelId = #{modelId}")
    int UpdateModel(UserModel model);

    @Delete("delete from dbModelInfo where modelId = #{modelId};")
    int DeleteModel(String modelId);
    @Select("select * from dbModelInfo where isShare = 1;")
